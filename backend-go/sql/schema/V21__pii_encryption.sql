...
SET blood_type_enc = pgp_sym_encrypt(blood_type::text, current_setting('app.encryption_key'))
WHERE blood_type IS NOT NULL
  AND blood_type_enc IS NULL;

UPDATE health_profiles
SET allergies_enc = pgp_sym_encrypt(array_to_string(allergies, ','), current_setting('app.encryption_key'))
WHERE allergies IS NOT NULL
  AND allergies_enc IS NULL;

UPDATE health_profiles
SET chronic_conditions_enc = pgp_sym_encrypt(array_to_string(chronic_conditions, ','), current_setting('app.encryption_key'))
WHERE chronic_conditions IS NOT NULL
  AND chronic_conditions_enc IS NULL;

UPDATE health_profiles
SET current_medications_enc = pgp_sym_encrypt(array_to_string(current_medications, ','), current_setting('app.encryption_key'))
WHERE current_medications IS NOT NULL
  AND current_medications_enc IS NULL;

UPDATE health_profiles
SET emergency_contact_name_enc = pgp_sym_encrypt(emergency_contact_name, current_setting('app.encryption_key'))
WHERE emergency_contact_name IS NOT NULL
  AND emergency_contact_name_enc IS NULL;

UPDATE health_profiles
SET emergency_contact_phone_enc = pgp_sym_encrypt(emergency_contact_phone, current_setting('app.encryption_key'))
...
SET diagnosis_enc = pgp_sym_encrypt(diagnosis, current_setting('app.encryption_key'))
WHERE diagnosis IS NOT NULL
  AND diagnosis_enc IS NULL;

UPDATE medical_records
SET prescription_enc = pgp_sym_encrypt(prescription, current_setting('app.encryption_key'))
WHERE prescription IS NOT NULL
  AND prescription_enc IS NULL;

UPDATE medical_records
SET doctor_name_enc = pgp_sym_encrypt(doctor_name, current_setting('app.encryption_key'))
WHERE doctor_name IS NOT NULL
  AND doctor_name_enc IS NULL;

UPDATE medical_records
SET notes_enc = pgp_sym_encrypt(notes, current_setting('app.encryption_key'))
...
UPDATE users
SET phone_enc = pgp_sym_encrypt(phone, current_setting('app.encryption_key'))
WHERE phone IS NOT NULL
  AND phone_enc IS NULL;

UPDATE users
SET email_enc = pgp_sym_encrypt(email, current_setting('app.encryption_key'))
WHERE email IS NOT NULL
  AND email_enc IS NULL;

UPDATE users
SET national_id_enc = pgp_sym_encrypt(national_id, current_setting('app.encryption_key'))
